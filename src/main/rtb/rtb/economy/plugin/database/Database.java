...
import rtb.economy.plugin.configuration.consts.Constants;
...
public class Database {
    ...
    private void database(Plugin instance) {
        boolean useMySQL = Config.getInstance().getConfigBoolean(Constants.USE_MYSQL);

        if (useMySQL) {
            String hostname = Config.getInstance().getConfigString(Constants.MYSQL_HOSTNAME);
            String databaseName = Config.getInstance().getConfigString(Constants.MYSQL_DATABASE);
            String username = Config.getInstance().getConfigString(Constants.MYSQL_USERNAME);
            String password = Config.getInstance().getConfigString(Constants.MYSQL_PASSWORD);
            int port = Config.getInstance().getConfigInt(Constants.MYSQL_PORT);
