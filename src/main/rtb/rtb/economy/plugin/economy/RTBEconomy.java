...
import static rtb.economy.plugin.configuration.consts.Constants.ECONOMY_DEFAULT_MONEY;
...
public class RTBEconomy {
    ...
    public boolean transferMoney(String player, BigDecimal money, TransactionType transactionType) {
        ...
        BigDecimal eventMoney = event.getMoney();

        switch (transactionType) {
            case REMOVE_ALLOW_BELLOW_ZERO -> playerAccount.subtract(eventMoney);
            case REMOVE -> {
                //Avoids people having less than 0 of "money"
                if (!playerAccount.hasEnoughMoney(eventMoney))
                    eventMoney = new BigDecimal(playerAccount.getMoneyPlainText());

                playerAccount.subtract(eventMoney);
            }
            case ADD -> playerAccount.add(eventMoney);
            case SET -> playerAccount.set(eventMoney);
            default -> System.out.println("Some error occurred ... This should not be happening");
        }
    ...
    public Collection<Account> getMoneyTop() {
        return this.moneyTop;
    }

    public void setMoneyTop(Collection<Account> moneyTop) {
