...
public class SubCmdSet extends AbstractSubCommand {
    ...
    public SubCmdSet(RTBEconomy economy) {
        super(economy, Config.getInstance().getCommandString("command.set.name"), Config.getInstance().getCommandString("command.set.usage"),
                Config.getInstance().getCommandString("command.set.permission"), Config.getInstance().getCommandsList("command.set.alias"));
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (NumberFormatterHelper.isFormatted(stringMoney)) {
            money = NumberFormatterHelper.stringToNumber(stringMoney);
        }
        ...
        sender.sendMessage(
                config.getMessage(MESSAGE_MODIFY_SET)
                        .replace(REPLACE_MONEY, NumberFormatterHelper.numberToString(money, formatNumber))
